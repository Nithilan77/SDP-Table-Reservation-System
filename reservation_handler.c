...
DateNode *date_list = NULL; /*Header node*/
...
void parse_input(char *data, char *name, char *email, char *guests, char *date, char *time) {
    ...
    while (token != NULL) {
        ...
        else if (strncmp(token, "guests=", 7) == 0) strcpy(guests, token + 7);
        else if (strncmp(token, "date=", 5) == 0) strcpy(date, token + 5);
        else if (strncmp(token, "time=", 5) == 0) strcpy(time, token + 5);
        token = strtok(NULL, "&");
    }
}
...
void save_to_text() {
    FILE *fp = fopen("C:\\xampp\\cgi-bin\\reservations.txt", "w");
    if (!fp) return;
...
int main() {
    ...
    if (!lenstr) {
        printf("<html><body><script>alert('Error: No data received'); window.location.href='/reservation.html';</script></body></html>");
        return 1;
    }
    ...
    printf("<script>");
    if (strcmp(status, "CONFIRMED") == 0) {
        printf("Swal.fire({icon: 'success', title: 'Reservation Confirmed!', text: 'Your table has been booked.', confirmButtonColor: '#3085d6'}).then(function(){ window.location.href='/reservation.html'; });");
    } else {
        printf("Swal.fire({icon: 'info', title: 'Added to Waitlist', text: 'All tables full. You are waitlisted.', confirmButtonColor: '#3085d6'}).then(function(){ window.location.href='/reservation.html'; });");
    }
    printf("</script>");
    printf("</body></html>");
